...
@pytest.fixture(scope='session')
def first_document() -> Document:
    return Document(id=UUID('31babbb3-5541-4a2a-8201-537cdff25fed'),
                    book_id=UUID('31babbb3-5541-4a2a-8201-537cdff25fed'),
                    customer='customer_1', create_date=datetime.now(),
                    doc='document_1')
...
@pytest.fixture(scope='session')
def second_document() -> Document:
    return Document(id=UUID('45309954-8e3c-4635-8066-b342f634252c'),
                    book_id=UUID('45309954-8e3c-4635-8066-b342f634252c'),
                    customer='customer_2', create_date=datetime.now(),
                    doc='document_2')
...
def test_get_document_by_id(first_document: Document, document_repo: DocumentRepo) -> None:
    assert document_repo.get_document_by_id(
...
def test_add_second_document(first_document: Document, second_document: Document, document_repo: DocumentRepo) -> None:
    ...
    documents = [document_repo.get_document_by_id(first_document.id),
                 document_repo.get_document_by_id(second_document.id)]
    assert len(documents) == 2
    assert documents[0] == first_document
    assert documents[1] == second_document


def test_delete_created_document(first_document: Document, second_document: Document, document_repo: DocumentRepo) -> None:
    assert document_repo.delete_document_by_id(
        first_document.id) == first_document
    assert document_repo.delete_document_by_id(
